  ...
  const handleBlockToggle = (u) => {
    ...
    setConfirm({
      ...
      onConfirm: async () => {
        setConfirm(null);
        try {
          isBlocked ? await api.unblockUser(u.user_id) : await api.blockUser(u.user_id);
          setUsers((prev) =>
            prev.map((x) =>
              x.user_id === u.user_id ? { ...x, status: isBlocked ? 'active' : 'blocked' } : x
            )
          );
  ...
  const handleDeleteListing = (l) => {
    setConfirm({
      ...
      onConfirm: async () => {
        setConfirm(null);
        try {
          await api.deleteListing(l.listing_id);
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-lg bg-white shadow-md overflow-x-auto">
                  <table className="w-full text-sm min-w-[700px]">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {filteredUsers.map((u) => (
                        <tr key={u.user_id} className={`hover:bg-gray-50 ${u.status === 'blocked' ? 'opacity-60' : ''}`}>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {orders.map((o) => (
                      <tr key={o.order_id} className="hover:bg-gray-50">
                        <td className="px-5 py-3 text-gray-500 font-mono text-xs">#{o.order_id}</td>
