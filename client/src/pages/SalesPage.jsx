        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filteredSales.map((sale) => (
            <div key={sale.orderId} className="rounded-lg bg-white shadow-md overflow-hidden hover:shadow-lg transition">
              ...
              <div className="p-4">
                <h3 className="font-semibold text-gray-800 mb-2 line-clamp-2">{sale.title}</h3>
                <p className="text-sm text-gray-600 mb-3 line-clamp-2">{sale.description}</p>
                ...
                <div className="mb-4 space-y-1">
                  ...
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Date:</span> {formatDate(sale.created_at)}
                  </p>
                </div>

                <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                  <span className="text-2xl font-bold text-green-600">+R{sale.amount?.toFixed(2) || '0.00'}</span>
                  <span className="text-sm font-medium text-gray-500">Order #{sale.orderId}</span>
                </div>
              </div>
            </div>
          ))}
